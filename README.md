...
1. Download the latest webapp snapshot jar of the validator from the artifacts generated by the Test Package workflow. See detailed instructions [here](DOWNLOAD_SNAPSHOT_JAR.md).
1. From the command line run `java -jar {JAR file name}`, where `{JAR file name}` is the name of the file you downloaded in the previous step. For example, if the JAR file name is `gtfs-realtime-validator-webapp-vsb89sdf8-SNAPSHOT.jar`, you would run `java -jar gtfs-realtime-validator-webapp-sb89sdf8-SNAPSHOT.jar`.
...
* `java -jar {JAR file name}`, where `{JAR file name}` is the name of the file you downloaded in the previous step. For example, if the JAR file name is `gtfs-realtime-validator-webapp-vsb89sdf8-SNAPSHOT.jar`, you would run `java -jar gtfs-realtime-validator-webapp-sb89sdf8-SNAPSHOT.jar`.
